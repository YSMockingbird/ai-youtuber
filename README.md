...
- AIにタイトル、説明文、配信構成、ニュース方針をまとめて作らせ、確認・編集してから開始する
...
配信内容の希望を入力するか空欄のまま「AIで配信案を作成」を押すと、AIがタイトル、
説明文、配信構成、ニュース方針を一度に作成します。内容を確認し、必要ならタイトルと
説明文を編集して「内容を設定して開始」を押してください。確認済みの配信構成をそのまま
開始後の発話へ再利用するため、同じ構成を作るためのLLM呼び出しは重複しません。

「内容を設定して開始」では、常設の「S のライブ配信」のタイトル・説明・公開設定を
更新してからOBSを開始します。開始可能な配信枠がない場合は、AIが作成したタイトル・
説明文・公開設定で新しいYouTube配信枠を自動作成し、既存の再利用ストリームへ接続します。
OBSの映像到着後にYouTubeをライブ状態へ移行するため、YouTube Studioで予約枠を
作成する必要はありません。管理画面では「AIで配信案を作成」から「内容を設定して開始」
の順で操作します。
